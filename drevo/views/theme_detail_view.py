...
from .theme_check_correctness_view import check_theme_correctness
from ..models import Znanie, IP, Visits, BrowsingHistory, Relation, Tr, ThemeAdditionalElements
from loguru import logger
from ..models.themes_data import ThemeWork, ThemeData
...
class ThemeDetailView(DetailView):
    ...
    template_name = "drevo/theme_detail.html"
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            context['previous_works'] = ThemeWork.objects.filter(user=self.request.user, theme=knowledge) \
                .values_list('work_name', flat=True)

            if previous_works := self.request.GET.get('previous_works'):
                context['progress'] = list(ThemeData.objects.filter(user=self.request.user, theme=knowledge,
            ...
            if not knowledge.several_works:
                context['progress'] = list(ThemeData.objects.filter(user=self.request.user, theme=knowledge,
                                                                    work__work_name='Данные по обучению').values(
                    'element', 'element_type'))
        ...
        try:
            ...
            next_relation = None
        try:
            start_of_theme = Relation.objects.get(bz=knowledge, tr__name='Начало обучения').rz
            context['theme_data'] = make_complicated_dict1(
                {'previous_key': []},
                start_of_theme,
                'previous_key',
            ...
            context['theme_data'] = []
        if check_theme_correctness(knowledge.id):
            context['warning_text'] = 'Данный обучение на данный момент находится в разработке'

        return context


def make_complicated_dict1(theme_dict, queryset, previous_key, level=1, next_relation=None):
    """
    ...
    if relations:
        if next_relation in relations.keys() and len(relations) == 1:
            if level == 0:
                theme_dict.append(queryset)
                for relation, elem in relations.items():
                    make_complicated_dict1(theme_dict, elem[0], elem[0], level=0)
            else:
                theme_dict[previous_key].append(queryset)
                for relation, elem in relations.items():
                    make_complicated_dict1(theme_dict[previous_key], elem[0], elem[0], level=0)
        else:
            if level == 0:
                theme_dict.append({queryset: []})
                last_direction = None
                for relation, elem in relations.items():
                    if relation.name == 'Далее':
                        ...
                        for el in elem:
                            make_complicated_dict1(theme_dict[len(theme_dict) - 1], el, queryset, level=1,
                                                   next_relation=next_relation)
                if last_direction:
                    make_complicated_dict1(theme_dict, last_direction, previous_key, level=0,
                                           next_relation=next_relation)
            else:
                theme_dict[previous_key].append({queryset: []})
                last_direction = None
                for relation, elem in relations.items():
                    if relation.name == 'Далее':
                        ...
                        for el in elem:
                            make_complicated_dict1(theme_dict[previous_key][len(theme_dict[previous_key]) - 1],
                                                   el, queryset, level=1, next_relation=next_relation)
                if last_direction:
                    make_complicated_dict1(theme_dict[previous_key], last_direction, previous_key, level=0,
                                           next_relation=next_relation)
    else:
        if level == 0:
            theme_dict.append(queryset)
...
class ThemeResultAdd(ProcessFormView):
    def get(self, request, pk, *args, **kwargs):
        if request.is_ajax():
            ...
            if pk:
                theme = get_object_or_404(Znanie, id=pk)
                ...
                previous_work = ThemeWork.objects.filter(theme=theme, user=user,
                                                         work_name=str(work_name)).first()

                if elements_for_deletion and elements_for_deletion == 'delete old results':
                    results_for_delete = ThemeData.objects.filter(work=previous_work)
                    results_for_delete.delete()
                else:
                    if not previous_work:
                        previous_work = ThemeWork.objects.create(
                            theme=theme,
                            user=user,
                            work_name=work_name,
                        )
                    ...
                    for deleted_element in elements_for_deletion:
                        elem = get_object_or_404(ThemeData, user=user, theme=theme,
                                                 work=previous_work, element=str(deleted_element))
                        elem.delete()

                    for theme_element, theme_element_type in saved_progress.items():
                        try:
                            next_relation = ThemeData.objects.get(theme=theme,
                                                                  user=user,
                                                                  element=str(theme_element), work=previous_work)
                            next_relation.element_type = theme_element_type
                            next_relation.save()
                        except ThemeData.DoesNotExist:
                            ThemeData.objects.create(
                                theme=theme,
                                user=user,
                                element=str(theme_element),
                                element_type=theme_element_type,
                                work=previous_work,
                            )
...
class EditTheme(ProcessFormView):
    def get(self, request, pk, *args, **kwargs):
        try:
            if request.is_ajax():
                user = self.request.user
                ...
                if pk:
                    theme = get_object_or_404(Znanie, id=pk)
                    work_name = request.GET.get('work')
                    previous_work, created = ThemeWork.objects.get_or_create(
                        theme=theme,
                        user=user,
                    ...
                    if request.GET.get('new_element'):
                        ...
                        ThemeAdditionalElements.objects.create(
                            user=user,
                            theme=theme,
                            work=previous_work,
                        ...
                        elem = get_object_or_404(ThemeAdditionalElements, user=user, theme=theme,
                        ...
                        elem.save()
                        results_for_redaction = ThemeData.objects.filter(work=previous_work,
