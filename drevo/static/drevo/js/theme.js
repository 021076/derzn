...
inside_themes = Array.from(document.querySelectorAll('.basic input[value="Тема"]'));
...
themes_and_chapters = inside_themes.concat(chapter);
...
function saveNewElement(){
    ...
    document.querySelector('#new_work_name').value).length > 0 || document.querySelector('#new_work_name').value == '')){
        ...
        spanTheme.setAttribute('class', 'theme-element ms-2');
        spanTheme.appendChild(aElement);
        spanTheme.appendChild(textNode);
        newLi.appendChild(spanText);
        newLi.appendChild(checkbox);
        newLi.appendChild(spanTheme);
...
function ShowFirst(){
    ...
    if(new_work && !(new_work == 'Данные по теме')){
    ...
    if(!(document.querySelector('.basic div#theme_tree span').classList.contains('text-secondary'))){
        showNotification(document.querySelector('.basic div#theme_tree span'), 'comment');
    }
    if(document.querySelector('.basic input[type="checkbox"]').parentNode.lastChild.tagName == 'UL' && document.querySelector('.basic input[type="checkbox"]').parentNode.lastChild.getElementsByTagName('li').length > 0){
        ...
        list_to_change_status.push([document.querySelector('.basic span.theme-element.ms-2 a').textContent, 'available']);
    }
    document.querySelectorAll('#theme_tree li[value="Далее"]').forEach((elem) => {
    ...
    if(isAuthenticated){
        setTimeout(()=>{
            $.ajax({
                ...
                url: document.location.pathname + '/theme_result/',
                success: function (response) {}
            });
        }, 1500);
    }
...
if((!urlParams.has('mode') || urlParams.get('mode') == '') && document.querySelector('#theme_tree')){
    ...
    if(!isManyWorks && (typeof getPreviousProgress() === 'undefined' || getPreviousProgress().length === 0)){
        setTimeout(()=>{new_work = 'Данные по теме';}, 1000);
        ShowFirst();
        ...
        if(document.querySelector('#work_name')){
            defolt_name = 1;
            while(Array.from(document.querySelectorAll('.select__item')).filter(item => item.textContent.trim() == 'Работа по теме '+String(defolt_name)).length
            > 0){
                defolt_name += 1;
            }
            document.querySelector('#work_name').value = 'Работа по теме '+String(defolt_name);
        }
    }
}else if(document.querySelector('#theme_tree')){
...
function findCheckbox(lay, name, previous_element){
    ...
    if(previous_element.parentNode.lastChild.tagName == 'UL'){
        previous_element.parentNode.lastChild.childNodes.forEach((child) => {
            if(!(child.firstChild && child.firstChild.style.display == 'none') && normalizeString(child.querySelector('.theme-element a').innerText) == normalizeString(name)){
                if(child.lastChild.tagName == 'UL' && child.lastChild.getElementsByTagName('li').length > 0){
                    lay = child.lastChild.childNodes;
                }
                founded_checkbox = child.querySelector('.theme-element').previousSibling
            }
        });
    }
    if(founded_checkbox == ''){
        while(founded_checkbox == ''){
            ...
            if(!(child.nodeName == '#text') && ((!(child.style.display == 'none') && !(child.firstChild && child.firstChild.style.display == 'none')) || child.getAttribute('value') == 'Вариант') && child.querySelector('.theme-element a').innerText == name){
                    if(child.lastChild.tagName == 'UL' && child.lastChild.getElementsByTagName('li').length > 0){
                        lay = child.lastChild.childNodes;
                    }
                    founded_checkbox = child.querySelector('.theme-element').previousSibling
        ...
        if(founded_checkbox == ''){
            elements_with_similar_name = Array.from(document.querySelectorAll('.theme-element a')).filter(item => normalizeString(item.innerText) == normalizeString(name));
            if(elements_with_similar_name.length > 1){
                elements_with_similar_name = elements_with_similar_name.filter(item => /green|blue/.test(item.parentNode.parentNode.parentNode.parentNode.querySelector('span.theme-element').style.color)
                || item.parentNode.parentNode.previousSibling && /green|blue/.test(item.parentNode.parentNode.previousSibling.querySelector('span.theme-element').style.color));
                if(elements_with_similar_name.length == 1){
...
function findIsExceptionType(previous_item, current_item){
    ...
    if(current_item.getAttribute('value') == 'Конец темы'){
        endTheTheme(current_item.parentNode);
    }
}
...
function recurseOpening(element){
    ...
    if(blocks_and_lists_of_variants.includes(element)){
    ...
    }else if(themes_and_chapters.includes(element)){
        ...
        if(!(element.parentNode.lastChild.querySelector('span').classList.contains('text-secondary'))){
            ...
            if(first_sub_elem.parentNode.lastChild.tagName == 'UL' && first_sub_elem.parentNode.lastChild.getElementsByTagName('li').length > 0){
                ...
                if(first_sub_elem.value == 'Конец темы'){
...
function startAction(action){
    ...
    if(action.checked == true){
        ...
        if(action.parentNode.lastChild.tagName == 'UL' && action.parentNode.lastChild.getElementsByTagName('li').length > 0){
            ...
            if(action.nextSibling.value == 'Конец темы'){
...
function findNextAction(next_action){
    ...
    if(next_action.querySelector('input[type="checkbox"]')){
        ...
        if(next_action.querySelector('input[type="checkbox"]').parentNode.lastChild.tagName == 'UL' && next_action.querySelector('input[type="checkbox"]').parentNode.lastChild.getElementsByTagName('li').length > 0){
            ...
            if(next_action.querySelector('input[type="checkbox"]').value == 'Конец темы'){
...
// Показывает уведомление о завершении темы или комментарий
function showNotification(elem, type){
    let list_to_change_status = [];
    if(type == 'ending'){
        let name_of_completed_theme = 'Вы прошли алгоритм "';
        const theme_to_end = document.querySelector('.container.header_info h1');
        const current_elem = elem
        while(name_of_completed_theme == 'Вы прошли алгоритм "'){
            if(!(elem.parentNode.parentNode.parentNode.tagName == 'LI')){
                name_of_completed_theme += document.querySelector('.container.header_info h1').textContent + '"';
                break;
            }
            if(elem.parentNode.parentNode.parentNode.querySelector('input[type="checkbox"].simple-elements').getAttribute('value') == 'Алгоритм'){
                ...
                name_of_completed_theme += full_name + '"';
            }else{
                elem = elem.parentNode.parentNode.parentNode.querySelector('input[type="checkbox"].simple-elements')
            }
        }
        document.querySelector('#end_of_theme').innerHTML = name_of_completed_theme;
        notificationPopup.style.display = 'block';
        setTimeout(()=>{removeNotification(closeNotificationButton)}, 10000);
        list_to_change_status = list_to_change_status.concat(endTheTheme(current_elem.parentNode));
    }else{
        ...
        document.querySelector('#end_of_theme').parentNode.before(comment);
...
function removeNotification(notification){
    if(notification.parentNode.classList.contains('end_of_theme')){
...
// При элементе "Конец темы" оставляем только путь элементов до него
function endTheTheme(action){
    let list_to_change_status = [];
