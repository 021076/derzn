...
from drevo.models import Znanie, Relation, QuestionToKnowledge, CourseAdditionalElements
...
@register.simple_tag
def recurse_dict(data, previous_knowledge, course, user=None, work=None, html='', display='block',
                 complicated_knowledge=None, mode=None, constructor=None):
    if constructor and course == previous_knowledge:
        html += f'<span class="course-element ms-2"><a href="{course.get_absolute_url()}">{course.name}</a></span>'
        html = constructor_mode_add_function(course, html)
        html = constructor_mode_redaction_functions(html, edit_function='editMainZnanie()',
                                                    delete_function=f'deleteMainZnanie(`course`, {course.id})')
        if data:
    ...
    for child in data:
        if isinstance(child, Znanie) or (isinstance(child, dict) and not list(child.values())[0]):
            ...
            if element_type != 'Комментарий':
                ...
                if constructor:
                    html = constructor_mode_redaction_functions(html, edit_function=f'edit_relation({rel.first().id})',
                                                                delete_function=f'delete_relation({parent.id}, {child.pk})')
                html += f'</span>'
                if course.several_works is False:
                    work = 'Данные по курсу'
                if user.is_authenticated and work:
                    if CourseAdditionalElements.objects.filter(user=user, work__work_name=work,
                                                               course=course,
                ...
                html += f'<input disabled type="checkbox" class="simple-elements" value="{element_type}" onclick="nextAction(this)">' \
                        f'<span class="course-element ms-2"><a href="{child.get_absolute_url()}">{child.name}</a> ' \
                        f'({element_type}){find_extra_span(child)}</span>'
                if mode and rel_name != 'Вариант' and not (
                        element_type != 'Блок' and rel_name in ['Можно сделать', 'Нужно сделать']):
            ...
            html += f'</li>'
            if user.is_authenticated and work:
                additional_elements = CourseAdditionalElements.objects.filter(user=user, work__work_name=work,
                                                                              course=course,
                                                                              parent_element=child).order_by(
                    'insertion_type')
            ...
            if list(child.keys())[0].tz.name != 'Комментарий':
                ...
                html += f'</span>' \
                        ...
                        f'<span class="course-element ms-2"><a href="{list(child.keys())[0].get_absolute_url()}">' \
                        f'{list(child.keys())[0]}</a> ({list(child.keys())[0].tz}){find_extra_span(list(child.keys())[0])}</span>'
                if mode:
                    if rel_name != 'Вариант' and not (
                            list(child.keys())[0].tz.name != 'Блок' and rel_name in ['Можно сделать', 'Нужно сделать']):
            ...
            if list(child.values())[0]:
                html = recurse_dict(list(child.values())[0], list(child.keys())[0], course, user=user, work=work,
                                    html=html, display='block' if constructor else 'none',
                                    complicated_knowledge=list(child.keys())[0], mode=mode, constructor=constructor)
            html += f'</li>'
            if course.several_works is False:
                work = 'Данные по курсу'
            if user.is_authenticated and work:
                additional_elements = CourseAdditionalElements.objects.filter(user=user, work__work_name=work,
                                                                              course=course,
                                                                              parent_element=list(child.keys())[
                                                                                  0]).order_by('insertion_type')
                if additional_elements:
                    html = add_user_elements(additional_elements, html, mode)
...
def add_user_elements(queryset, html, mode):
    ...
    for new_elem in queryset:
        common_part = f'<input disabled type="checkbox" class="simple-elements" value="Действие" onclick="nextAction(this)">' \
                      f'<span class="course-element ms-2"><a class="new-element">{new_elem.element_name}</a> (Действие)</span>'
        if mode:
