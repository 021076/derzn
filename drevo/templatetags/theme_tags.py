...
from drevo.models import Znanie, Relation, QuestionToKnowledge, ThemeAdditionalElements
...
@register.simple_tag
def recurse_dict(data, previous_knowledge, theme, user=None, work=None, html='', display='block',
                 complicated_knowledge=None, mode=None, constructor=None):
    if constructor and theme == previous_knowledge:
        html += f'<span class="theme-element ms-2"><a href="{theme.get_absolute_url()}">{theme.name}</a></span>'
        html = constructor_mode_add_function(theme, html)
        html = constructor_mode_redaction_functions(html, edit_function='editMainZnanie()', delete_function=f'deleteMainZnanie(`theme`, { theme.id })')
        if data:
    ...
    for child in data:
        if isinstance(child, Znanie) or (isinstance(child, dict) and not list(child.values())[0]):
            ...
            if element_type != 'Комментарий':
                ...
                html += f'</span>'
                if theme.several_works is False:
                    work = 'Данные по теме'
                if user.is_authenticated and work:
                    if ThemeAdditionalElements.objects.filter(user=user, work__work_name=work,
                                                                                     theme=theme,
                ...
                html += f'<input disabled type="checkbox" class="simple-elements" value="{element_type}" onclick="nextAction(this)">'\
                        f'<span class="theme-element ms-2"><a href="{child.get_absolute_url()}">{child.name}</a> ' \
            ...
            html += f'</li>'
            if user.is_authenticated and work:
                additional_elements = ThemeAdditionalElements.objects.filter(user=user, work__work_name=work, theme=theme,
            ...
            if list(child.keys())[0].tz.name != 'Комментарий':
                ...
                html += f'</span>'\
                        ...
                        f'<span class="theme-element ms-2"><a href="{list(child.keys())[0].get_absolute_url()}">' \
            ...
            if list(child.values())[0]:
                html = recurse_dict(list(child.values())[0], list(child.keys())[0], theme, user=user, work=work,
...
def add_user_elements(queryset, html, mode):
    ...
    for new_elem in queryset:
        common_part = f'<input disabled type="checkbox" class="simple-elements" value="Действие" onclick="nextAction(this)">' \
                      f'<span class="theme-element ms-2"><a class="new-element">{new_elem.element_name}</a> (Действие)</span>'
        if mode:
